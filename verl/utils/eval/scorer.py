...
class AutoScoringJudge:
    ...
    def split_by_comma(self, expr: str) -> list[str]:
        """Split expressions by comma outside of brackets."""
        in_bracket_num = 0
        splitted_expr = []
        start_idx = 0
    ...
    def judge(
        self, ground_truth: str, prediction: str, precision: Optional[float] = None
    ) -> bool:
        """
    ...
    def is_equal(self, expression1: str, expression2: str, precision: float) -> bool:
        ...
        if _init_sympy():
            try:
                if self.expression_equal(expression1, expression2, precision) and not (
    ...
    def mcq_equal(self, expression1: str, expression2: str) -> bool:
        ...
        if expression1.upper() in ["A", "B", "C", "D", "E"]:
            ...
            if (
                re.match(r"[ABCDEabcde][)\.]*\s.*", expression2)
                and expression2[0].upper() == expression1.upper()
            ):
                return True
    ...
    def numerical_with_unit_equal(
        self, expression1: str, expression2: str, precision: float
    ) -> bool:
    ...
    def interval_equal(
        ...
        def compare_two_interval(inter1: str, inter2: str) -> bool:
            ...
            inter1 = inter1.strip("[]()").strip()
            inter2 = inter2.strip("[]()").strip()
