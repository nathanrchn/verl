...
def math_task(
    output: dict[str, Any] | list[dict[str, Any]], params: RolloutParams
) -> dict[str, float]:
    """
...
def code_task(outputs: list[dict[str, Any]], params: RolloutParams) -> dict[str, float]:
    ...
    output_texts = [output.get("text", "") for output in outputs]
    ...
    if not metrics:
        logger.warning(
            f"No pass@k results found in code evaluation for task {task_name}. "
            f"Results: {pass_at_k}. Predictions: {predictions} ({n_samples} samples)"
        )
...
def code_thinking_task(
    outputs: list[dict[str, Any]], params: RolloutParams
) -> dict[str, float]:
    """
    ...
    if f"{task_name}/accuracy" not in metrics and f"{task_name}/pass@10" not in metrics:
        logger.warning(
            f"No pass@k results found in code evaluation for task {task_name}. "
            f"Results: {pass_at_k}. Predictions: {extracted_code} ({n_samples} samples)"
        )
        metrics[f"{task_name}/accuracy"] = 0.0
...
def ifeval_task(outputs: list[dict[str, Any]], params: RolloutParams) -> dict[str, float]:
    ...
    """
    try:
        from verl.utils.eval.benchmarks.ifeval.instructions_registry import (
            INSTRUCTION_DICT as IFEVAL_INSTRUCTION_DICT,
        )
    except ImportError:
        logger.warning("IFEval instruction registry not found, returning 0.0 accuracy")
        return {"ifeval/accuracy": 0.0}

    output_texts = [parse_non_reasoning_apertus(output.get("text", "")) for output in outputs]
    ...
    for output_text in output_texts:
        output_results = []
        for instruction_id, instruction_kwargs in zip(instruction_id_list, instruction_kwargs_list):
    ...
    return {"ifeval/accuracy": float(sum(results) / len(results)) if results else 0.0}
...
def ifbench_task(outputs: list[dict[str, Any]], params: RolloutParams) -> dict[str, float]:
    ...
    """
    try:
        from verl.utils.eval.benchmarks.ifbench.instructions_registry import (
            INSTRUCTION_DICT as IFBENCH_INSTRUCTION_DICT,
        )
    except ImportError:
        logger.warning("IFBench instruction registry not found, returning 0.0 accuracy")
        return {"ifbench/accuracy": 0.0}
    ...
    return {"ifbench/accuracy": float(sum(results) / len(results)) if results else 0.0}
...
TASK_REGISTRY: dict[
    str,
    Callable[[Union[dict[str, Any], list[dict[str, Any]]], RolloutParams], dict[str, float]],
] = {
